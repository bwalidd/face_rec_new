...
export const UsersTable = () => {
  ...
  useEffect(() => {
    (async () => {
      const res = await axios.get(`/overview/detailed_extra/${id}/${start}/${end}/${zones}/${filter_range}?page=${page}`);
      setDetections(res.data.results);
      setCount(Math.ceil(res.data.count / 20));
    })();
  }, [page, id, start, end, zones]);
    ...
    <>
      ...
      <div className="flex items-center justify-center">
        <Pagination
          ...
          initialPage={1}
          onChange={(p) =>
            navigate(`/face-recognition/detections/${p}/${id}/${start}/${end}/${zones}/${filter_range}`)
          }
        />
      </div>
